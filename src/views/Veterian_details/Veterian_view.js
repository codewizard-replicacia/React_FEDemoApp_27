...
const ViewVeterian = () => {
  ...
  const [veterian, setVeterian] = useState({});
  ...
  useEffect(() => {
    if (id) {
      const fetchVeterianById = async () => {
        ...
        const VeterianJsonResp = await VeterianResponse.json();
        setVeterian(VeterianJsonResp);
      };
      fetchVeterianById();
    }
  }, [id]);
          ...
          <Grid>
            <Grid item lg={12} xs={12}>
              <Box display="flex" justifyContent="space-between">
                <Typography className="page-heading" variant="h5">View Veterian</Typography>
                                  ...
                                  <TableCell className={[styles.valueCell, 'row-value'].join(' ')}>
                                    {
                                      ...
                                      <Typography variant="body1">
                                        {veterian[key] !== null &&
                                          moment.utc(veterian[key]).format(
